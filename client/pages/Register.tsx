...
export default function Register() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ name: "", age: "", gender: "male", bloodGroup: undefined, city: "", pincode: "", mobile: "" });
    } catch (e: any) {
    ...
    <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <form onSubmit={submit} className="space-y-4">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Input placeholder="Age" type="number" min={18} max={65} value={form.age} onChange={(e)=>setForm({...form,age:e.target.value})} required />
        </div>
        <div>
          <Label className="text-sm">Gender</Label>
          <RadioGroup className="mt-2 grid grid-cols-3 gap-2" value={form.gender} onValueChange={(v)=>setForm({...form, gender:v as Gender})}>
            ...
            <div className="flex items-center space-x-2"><RadioGroupItem value="other" id="g-o" /><Label htmlFor="g-o">Other</Label></div>
          </RadioGroup>
        </div>
        <div className="grid gap-4 md:grid-cols-3">
          <Select onValueChange={(v)=>setForm({...form, bloodGroup:v as BloodGroup})}>
        ...
        <Button type="submit" disabled={loading}>{loading?"Submitting...":"Register as Donor"}</Button>
      </form>
    </div>
  );
}
