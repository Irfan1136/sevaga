  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background py-8 px-4">
      <div className="w-full max-w-md bg-card border rounded-2xl p-6 shadow-md">
        <h1 className="text-2xl font-extrabold text-center text-primary mb-2">Enter OTP</h1>
        <p className="text-sm text-muted-foreground text-center mb-4">Enter the OTP sent to your mobile/email to verify your account.</p>
        <form onSubmit={submit} className="space-y-4">
          <Input placeholder="6-digit OTP" value={otp} onChange={(e)=>setOtp(e.target.value.replace(/[^0-9]/g, '').slice(0,6))} inputMode="numeric" maxLength={6} />
