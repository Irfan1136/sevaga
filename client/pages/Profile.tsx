    ...
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="rounded-lg border bg-card p-6">
        <div className="grid gap-2">
          ...
          <div><strong>Account:</strong> {account.type}</div>
          <div><strong>Email:</strong> {account.email || '—'}</div>
          <div><strong>Mobile:</strong> {account.mobile || '—'}</div>
          <div><strong>Verified At:</strong> {account.verifiedAt ? new Date(account.verifiedAt).toLocaleString() : '—'}</div>
        </div>
      </div>
      ...
      {donor && (
        <div className="mt-6 rounded-lg border bg-card p-6">
          ...
          <div className="grid gap-2">
            ...
            <div><strong>Mobile:</strong> {donor.mobile}</div>
          </div>
        </div>
      )}
    </div>
